 ...
 */
public class OpenGrade extends JDialog {
	...
	protected JButton btn_remove = new JButton("Poista tulos");
	 ...
	 * @param exam Arvosanan koe.
	 * @param modality Modaalisuuden tyyppi.
	 */
	public OpenGrade(Controller controller, Grade grade, Exam exam, ModalityType modality) {
	...
	private void init() {
		panel_buttons.add(btn_remove);
		btn_remove.addActionListener(gradeRemove);

		textField_grade.setText(grade.getFieldAsString(FN.GRADE_GRADE));

		JTable table = studentsView.table_data;
		table.setModel(controller.getStudentTableModel("", ""));
		table.setAutoCreateRowSorter(true);

		IUnit stu = controller.getRefTarget(grade, FN.GRADE_STUDENT_REF);
		DataTableModel<Student> dtm = (DataTableModel<Student>)table.getModel();
		int row = dtm.getRow((Student)stu);
		int screenRow = table.convertRowIndexToView(row);
		studentsView.table_data.setRowSelectionInterval(screenRow, screenRow);
	...
	private ActionListener gradeRemove = new ActionListener() {

		@Override
		public void actionPerformed(ActionEvent e) {
			if (controller.remove(grade)) {
				dispose();
			}
		}
	};
