...
function listaaReseptit(data, status, request) {
    var $reseptilista = $("#reseptilista");
    $reseptilista.empty();
    var avaaResepti = function (event) {
        event.preventDefault();
        var $a = $(event.target);
        var reseptinId = $a.data("id");
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        var ohjeet = resepti["ohjetekstit"];
...
function alustaReseptiLomake() {
    ...
    var handleInput = function (callback) {
    ...
    $("#resepti_nimi").on("input", function (event) {
        handleInput(function (data) {
            $("#resepti_nimi_err").text(data["nimi"]);
        });
    });
    $("#resepti_maara").on("input", function (event) {
        handleInput(function (data) {
...
function handlaaReseptiVirheet(virheet) {
...
function taytaLajiValikko(data, status, request) {
...
function reseptiLisatty(data, status, request) {
...
function naytaReseptinMuokkaus(reseptinId) {
    naytaSisalto("add_recipe_templ");
...
function naytaResepti(data, status, request) {
    var resepti = data[0];
...
function tallennaResepti(reseptinId) {
    var form = $("#lisayslomake").formSerialize();
    form += "&id=" + reseptinId;
    $.ajax({
        async: true,
...
function reseptiTallennettu(data, status, request) {
    var $status = $("#recipe_status_text");
    if (data == "") {
        $status.removeClass("errortext");
...
function listaaLajit(data, status, request) {
    var $lajit = $("#lajit");
    $lajit.empty();
    for (var i = 0; i < data.length; i++) {
        var laji = data[i];
...
function handlaaLajiInput(event) {
    ...
    var $field = $(field);
    tarkistaLaji(field, function (data, status, request) {
...
function tarkistaLaji(srcField, callback) {
...
function tallennaLaji(event) {
    ...
    var form = $nappi.closest("form").formSerialize();
    form += "&id=" + id;
    $.ajax({
        async: true,
...
function lajiTallennettu(field, data, status, request) {
...
function lisaaLaji(event) {
    ...
    var form = $("#uusi_laji_lomake").formSerialize();
    $.ajax({
        async: true,
        url: "lisaa_laji.py",
        data: form,
...
// Näytä tilaviesti lajin tallennuksen tai yrityksen jälkeen, pyyhi aiemmat tilaviestit.
function lajiLisatty(data, status, request) {
    tyhjennaLajiStatukset();
